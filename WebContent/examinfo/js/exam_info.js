...
$(document).ready(function() {
    ...
    if(Number(total_count) > 0){
        ...
        $.jqPaginator('#pagination', {
            ...
            onPageChange: function (num, type) {
                if(type=="change"){
                    window.location.href="getExamInfoDataList.action?page="+num;
                }
            }
        });
  ...
  //新增-保存
    $("#save_exam_info").click(function() {
        ...
        //提交表单
        $("#addExamInfoForm").submit();
    });
    
  //编辑保存
    $("#update_exam_info").click(function() {
    	if ($("#ti_name").val() == '') {
            alert("请输入培训机构名称!");
            return false;
        }
...
function toTeacherInfoList(id){
	window.location.href = "getTeacherInfoDataList.action?examInfoId="+id;
}

//课程信息列表
function toCourseInfoList(id){
    window.location.href = "getCourseInfoDataList.action?examInfoId="+id;
}
...
function toAdd(){
	window.location.href = "toAddExamInfo.action";
}
function toInput(){
	window.location.href = "toExamInfoInput.action";
}
//编辑
function toEdit(id){
	window.location.href = "toEditExamInfo.action?examInfo.id="+id;
}

//已考试学生
function toExamRecordList(examId){
	if(examId == null || examId==''){
		layer.msg("数据错误");
		return;
	}
	window.location.href = "getExamDataListByExamId.action?userExam.examInfo.id="+examId;
}
